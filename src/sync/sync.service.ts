...
export class SyncService {
      ...
      for (const row of dirty) {
        ...
        if (safeTable === 'local_expenses' && row.receipt_local && !row.receipt_url) {
          try {
            ...
            const base64 = await LegacyFileSystem.readAsStringAsync(row.receipt_local, {
        ...
        try {
          if (row._deleted) {
            if (row._server_id) {
              await this.supa.from(remoteTable).update({ deleted_at: new Date().toISOString() }).eq('id', row._server_id);
            }
          ...
          if (ok) {
            await this.db.runAsync(`UPDATE ${safeTable} SET _dirty = 0, _sync_status = 'synced' WHERE _local_id = ?`, [row._local_id]);
          }
        } catch (err: any) {
  ...
  private async pull() {
    ...
    const cursorRow = await this.db.getFirstAsync<{ value: string } | undefined>(
